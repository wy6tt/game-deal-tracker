...
function Free({ user, wishlist, setWishlist }) {
  ...
  useEffect(() => {
    fetch("https://www.cheapshark.com/api/1.0/deals?upperPrice=0&pageSize=60&sortBy=Deal Rating")
      .then(res => res.json())
      .then(data => {
        const seen = new Set()
    ...
    <main className="max-w-6xl mx-auto px-6 py-10">
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="bg-gray-900 rounded-2xl overflow-hidden border border-gray-800 animate-pulse">
              <div className="w-full h-24 sm:h-32 bg-gray-800" />
        ...
        <div className="text-center py-24">
          ...
          <p className="text-gray-400">Check back soon — free deals pop up all the time!</p>
        </div>
      ) : (
        <>
          {deals.length < 4 && (
            <div className="bg-yellow-950 border border-yellow-800 rounded-2xl px-5 py-4 mb-6 text-sm text-yellow-300">
              🎁 Only {deals.length} free game{deals.length === 1 ? "" : "s"} available right now — check back soon for more!
            </div>
          )}
          <GameGrid items={deals} />
        </>
      )}
    </main>
  )
